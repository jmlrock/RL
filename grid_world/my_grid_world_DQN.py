...
#        self.summary=self.model.summary()
    ...
    def act(self, state):
        ...
        return np.argmax(self.model.predict(np.array([state]))[0]) #ok
...
#        self.target_model.compile(loss="mean_squared_error", optimizer=Adam(lr=0.1))
        ...
        for sample in samples:
            ...
            if done:
                ...
                Q_future = np.max(self.target_model.predict(np.array([new_state])))
                target[0][action] = reward + Q_future * self.gamma
            self.model.fit(np.array([state]), target, epochs=1, verbose=0) #1 seul update 
...
for e in range(epoch):
    ...
    for s in range(1,10):
        Q=dqn_agent.model.predict(np.array([s]))
        print(s,Q)
