# -*- coding: utf-8 -*-
"""
Created on Wed Nov 13 16:49:35 2019
...
class Visualizer:
    ...
    def plot_a_episode(self,env, model,explored_cum_rewards, explored_actions, 
...
#        plt.close()
    ...
    def plot_episodes(self, 
        ...
        ma=pd.DataFrame(safe_total_rewards).rolling(MA_window).mean()
#        ma=np.roll(np.array(safe_total_rewards),MA_window.mean()
#        std = pd.rolling_std(np.array(safe_total_rewards), window=MA_window, min_periods=3)
        std = pd.DataFrame(safe_total_rewards).rolling(MA_window).std()
