# -*- coding: utf-8 -*-
"""
Created on Wed Nov 27 14:30:59 2019
...
window_state=5
environment = Trading_env(prices,window_state) 
action_size = environment.n_action  
state = np.array(environment.reset(test=True))
...
if (train_test == 'train'):
    for _ in range(memory_size):
        action_encoded = agent.act(state)
        action=valid_action[np.argmax(action_encoded)]
        next_state, reward, done = environment.step(action)
        agent.observe(state, action_encoded, reward, next_state, done, warming_up=True)
        if done:
            state = np.array(environment.reset(test=True))
...
if train_test == "train":
    ...
    for ep in range(episodes):
        ms = time.time()
        state = environment.reset(test=True)
        rew = 0
        loss_list_temp = []
        val_loss_list_temp = []
        ...
        for _ in range(episode_length):
            ...
            action=valid_action[np.argmax(action_encoded)]
            next_state, reward, done = environment.step(action)
            loss = agent.observe(state, action_encoded, reward, next_state,done) 
            state = next_state
            rew += reward
            if(loss):
