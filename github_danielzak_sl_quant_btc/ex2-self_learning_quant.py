...
#Load data
def load_data():
    price = np.sin(np.arange(200)/5) #sine prices
    return price
...
#indata = load_data()
df=prices=pd.read_csv('CAC.csv',index_col='Date',parse_dates=True)
df=pd.read_csv('CAC.csv',index_col='Date',parse_dates=True).rolling(2).mean()
prices=df['Adj Close']
prices=prices.dropna()
indata=np.array(prices.iloc[200:900])
plt.plot(indata)
epochs = 50
