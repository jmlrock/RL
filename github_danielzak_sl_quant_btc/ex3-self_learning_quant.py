...
prices.rename(columns={'Open': 'open', 'High': 'high', 'Low': 'low', 'Close': 'close','Adj Close': 'adj', 'Volume': 'volume'}, inplace=True)
x_train = prices.iloc[2300:2500,]
x_test= prices.iloc[2500:3000]
...
state, xdata, close=init_state(prices, test=False)
...
def evaluate_Q(eval_data, eval_model, price_data, epoch=0):
    ...
    while(status == 1):
        ...
        qval = eval_model.predict(state.reshape(1,1,2), batch_size=1)
...
model = Sequential()
model.add(Dense(4, input_shape=(None,2,),activation='relu'))
#model.add(Dropout(0.2)) I'm not using dropout in this example

model.add(Dense(4,activation='relu'))
#model.add(Dropout(0.2))

model.add(Dense(4,activation='linear')) #linear output so we can have range of real-valued outputs

rms = RMSprop()
model.compile(loss='mse', optimizer=rms)


s=state.reshape(1,1,2)
a=np.ones((10,1,2))
qval = model.predict(s,batch_size=1)
print(qval)
...
#        indata = load_data(test=True)
    ...
    while(status == 1):
        ...
        qval = model.predict(state.reshape(1,1,2), batch_size=1)
        ...
        if (len(replay) < buffer): #if buffer not filled, add to it
            ...
            for memory in minibatch:
                ...
                old_qval = model.predict(old_state.reshape(1,1,2), batch_size=1)
                newQ = model.predict(new_state.reshape(1,1,2), batch_size=1)
                maxQ = np.max(newQ)
                y = np.zeros((1,4))
                y[:] = old_qval[:]
            ...
            for i in range(len(X_train)):
                X_train[i]=X_train[i].reshape(1,1,2)
                
            
            y_train = np.array(y_train).reshape(100,1,4)
